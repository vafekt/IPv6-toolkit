...
def parameter():
    ...
    return args.interface, args.source_mac, args.source_ip, args.destination_ip, args.hop_lim, args.num_packets, data, args.flood


def generate(interface, source_mac, source_ip, destination_ip, hop_lim, num_packets, data, flood):
    # Generate the packet
    id = RandShort()
    ...
    if flood is None:  # Normal situation
        ...
        for i in range(num_packets):
            packet1 = Ether(src=source_mac) / IPv6(src=source_ip, dst=destination_ip, hlim=hop_lim) / ICMPv6EchoRequest(id=id, seq=i + 1, data=data)
    ...
    if flood is not None:  # Flooding the target
        ...
        if flood == "constant":
            for i in range(150):
                packet1 = Ether(src=source_mac) / IPv6(src=source_ip, dst=destination_ip,
...
def execute_functions(interface, source_mac, source_ip, destination_ip, hop_lim, num_packets, data, flood):
    p1 = multiprocessing.Process(target=generate, args=[interface, source_mac, source_ip, destination_ip, hop_lim, num_packets, data, flood])
...
if __name__ == "__main__":
    interface, source_mac, source_ip, destination_ip, hop_lim, num_packets, data, flood = parameter()
    if flood is None:
        try:
            execute_functions(interface, source_mac, source_ip, destination_ip, hop_lim, num_packets, data, flood)
        except KeyboardInterrupt:
            sys.exit(0)
    else:
        try:
            generate(interface, source_mac, source_ip, destination_ip, hop_lim, num_packets, data, flood)
        except KeyboardInterrupt:
