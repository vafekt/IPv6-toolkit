...
def parameter():
    ...
    return args.interface, args.source_mac, args.source_ip, args.destination_ip, args.hops, args.data_length, args.flood


def generate(interface, source_mac, source_ip, destination_ip, hops, data_length, flood):
    # Generate packet
...
def execute_functions(interface, source_mac, source_ip, destination_ip, hops, data_length, flood):
    p1 = multiprocessing.Process(target=generate, args=[interface, source_mac, source_ip, destination_ip, hops, data_length, flood])
...
if __name__ == "__main__":
    interface, source_mac, source_ip, destination_ip, hops, data_length, flood = parameter()
    if flood is None:
        try:
            execute_functions(interface, source_mac, source_ip, destination_ip, hops, data_length, flood)
        except KeyboardInterrupt:
            sys.exit(0)
    else:
        try:
            generate(interface, source_mac, source_ip, destination_ip, hops, data_length, flood)
        except KeyboardInterrupt:
