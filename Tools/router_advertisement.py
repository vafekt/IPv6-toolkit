...
def main():
    ...
    parser.add_argument("-pref", dest="preference", action="store", choices=['High', 'Medium', 'Low', 'Reserved'],
                        default='Medium', help="the preference level of default router (Medium if skipping).")
    parser.add_argument("-rlt", dest="router_lifetime", action="store", type=int,
                        default=1800, help="the router lifetime in seconds (1800 if skipping).")
    parser.add_argument("-rcht", dest="reachable_time", action="store", type=int,
                        default=30000, help="the router lifetime in milliseconds (30000 if skipping).")
    parser.add_argument("-rtrt", dest="retrans_timer", type=int, action="store", default=0,
                        help="the retransmission timer in milliseconds (0 if skipping).")
    parser.add_argument("-prefix", dest="prefix_info", action="store", help="the prefix information of router (not "
                                                                            "included if skipping).")
    parser.add_argument("-vlt", dest="valid_lftime", action="store", type=int, default=4294967295,
                        help="the valid lifetime of prefix in seconds (infinity if skipping).")
    parser.add_argument("-plt", dest="prefered_lftime", action="store", type=int, default=4294967295,
                        help="the preferred lifetime of prefix in seconds (infinity if skipping).")
    ...
    if args.destination_mac is not None:
        layer2 = Ether(src=args.source_mac, dst=args.destination_mac)
        ...
        if args.flood == "random":
            for i in range(100):
                ...
                for m in range(num_routeInfo):
                    ...
                    packet1 /= ICMPv6NDOptRouteInfo(plen=64, prf=args.preference, prefix=network, rtlifetime=args.valid_lftime)
