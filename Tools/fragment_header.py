...
def parameter():
    ...
    parser.add_argument("-frag", dest="frag_hdr", action="store", type=int,
                        help="the number of fragment headers (1 if skipping).")
    parser.add_argument("-id", dest="id", action="store_true", default=False,
                        help="set fragment headers to have the same ID (different ID if skipping).")
    parser.add_argument("-l", dest="data_length", type=int, action="store",
                        help="the size of data in bytes (32 if skipping).")
    parser.add_argument("-tiny", dest="tiny_frag", action="store_true", default=False, help="send tiny fragments.")
    ...
    return args.interface, args.source_mac, args.destination_mac, args.source_ip, args.destination_ip, args.frag_hdr, args.id, args.data_length, args.tiny_frag, args.overlap_frag, args.flood


def generate(interface, source_mac, destination_mac, source_ip, destination_ip, num_frag_hdr, id, data_length, tiny_frag, overlap_frag, flood):
    # Generate packet
...
def execute_functions(interface, source_mac, destination_mac, source_ip, destination_ip, num_frag_hdr, id, data_length, tiny_frag, overlap_frag, flood):
    p1 = multiprocessing.Process(target=generate, args=[interface, source_mac, destination_mac, source_ip, destination_ip, num_frag_hdr, id, data_length, tiny_frag, overlap_frag, flood])
...
if __name__ == "__main__":
    interface, source_mac, destination_mac, source_ip, destination_ip, num_frag_hdr, id, data_length, tiny_frag, overlap_frag, flood = parameter()
    if flood is None:
        try:
            execute_functions(interface, source_mac, destination_mac, source_ip, destination_ip, num_frag_hdr, id, data_length, tiny_frag, overlap_frag, flood)
        except KeyboardInterrupt:
            sys.exit(0)
    else:
        try:
            generate(interface, source_mac, destination_mac, source_ip, destination_ip, num_frag_hdr, id, data_length, tiny_frag, overlap_frag, flood)
        except KeyboardInterrupt:
