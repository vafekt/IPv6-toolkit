...
def parameter():
    ...
    parser.add_argument("-dmac", dest="destination_mac", action="store",
                        help="the MAC address of receiver (33:33:00:01:00:02 if skipping).")
    ...
    parser.add_argument("-rc", dest="rapid_commit", action="store_true", default=False,
                        help="activate the Rapid Commit for possibly immediate Reply from server instead of Advertise "
                             "messages.")
    ...
    if args.destination_mac is None:
        args.destination_mac = "33:33:00:01:00:02"
    ...
    return args.interface, args.source_mac, args.destination_mac, args.source_ip, args.destination_ip, args.rapid_commit, args.trid, args.iaid, args.duid, args.fqdn, args.request, args.period, args.flood


def generate(interface, source_mac, destination_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period,
             flood):
    # Generate the packet
    layer2 = Ether(dst=destination_mac, src=source_mac)
...
def sniffing(interface, source_mac, destination_mac, source_ip, destination_ip, fqdn, request):
...
def sniffing_forever(interface, source_mac, destination_mac, source_ip, destination_ip, fqdn, request):
    # Define our Custom Action function
    packet_counts = Counter()
...
def execute_functions(interface, source_mac, destinatino_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood):
    p1 = multiprocessing.Process(target=generate, args=[interface, source_mac, destinatino_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood])
    p2 = multiprocessing.Process(target=sniffing, args=[interface, source_mac, destinatino_mac, source_ip, destination_ip, fqdn, request])
    p2.start()
    p1.start()
    p2.join()
    p1.join()
...
def execute_functions_forever(interface, source_mac, destination_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood):
    p1 = multiprocessing.Process(target=generate, args=[interface, source_mac, destination_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood])
    p2 = multiprocessing.Process(target=sniffing_forever, args=[interface, source_mac, destination_mac, source_ip, destination_ip, fqdn, request])
    p2.start()
    p1.start()
    p2.join()
    p1.join()
...
if __name__ == "__main__":
    interface, source_mac, destination_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood = parameter()
    if flood is None and period is None:
        try:
            execute_functions(interface, source_mac, destination_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood)
        except KeyboardInterrupt:
            sys.exit(0)
    elif period is not None:
        try:
            execute_functions_forever(interface, source_mac, destination_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood)
        except KeyboardInterrupt:
            sys.exit(0)
    else:
        try:
            generate(interface, source_mac, destination_mac, source_ip, destination_ip, rapid_commit, trid, iaid, duid, fqdn, request, period, flood)
        except KeyboardInterrupt:
