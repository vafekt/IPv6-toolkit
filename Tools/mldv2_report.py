...
def parameter():
    ...
    return args.interface, args.source_mac, args.source_ip, args.destination_ip, list_rtype, list_mip, list_src, args.num_packets, args.period, args.flood


def generate(interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood):
    # Generate packet
...
def execute_functions(interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood):
    p1 = multiprocessing.Process(target=generate,
                                 args=[interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood])
...
def execute_functions_forever(interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood):
    p1 = multiprocessing.Process(target=generate,
                                 args=[interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood])
...
if __name__ == "__main__":
    interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood = parameter()
    if flood is None and period is None:
        try:
            execute_functions(interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood)
        except KeyboardInterrupt:
            sys.exit(0)
    elif period is not None:
        try:
            execute_functions_forever(interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood)
        except KeyboardInterrupt:
            sys.exit(0)
    else:
        try:
            generate(interface, source_mac, source_ip, destination_ip, list_rtype, list_mip, list_src, num_packets, period, flood)
        except KeyboardInterrupt:
