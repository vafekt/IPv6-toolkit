...
def main():
    ...
    parser.add_argument("-p", dest="period", type=int, help="launch the detection tool every specified second.")
    ...
    def custom_action(packet):
        ...
        if packet[0][1].src not in ip_list:  # Deleting duplicate address when capturing
            ...
            if ICMPv6ParamProblem in packet:
                return f"+ Discovered the IPv6 address number #{sum(packet_counts.values())}: " \
